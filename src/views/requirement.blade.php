...
@section('content')
    ...
    <div class="content-body">
        <ul class="list-group mt-3">
            @foreach($extensionArrayList as $key => $extension)
                @if($loop->first)
                    <li class="list-group-item d-flex align-items-center justify-content-between bg-primary text-white">
                ...
                @else
                    <li class="list-group-item d-flex align-items-center justify-content-between">
                        <span>{{ $key }}</span>
                        <i class="{{$extension ==1 ? 'text-success fa fa-check-square':'text-danger fa fa-times'}}" aria-hidden="true"></i>
                    </li>
                @endif
            @endforeach
        </ul>
        @if(!in_array(0, $extensionArrayList))
            <a class="btn-proceed" href="{{ route('permissions') }}">Check Permissions&nbsp;<i class="fa fa-angle-right" aria-hidden="true"></i></a>
        @else
